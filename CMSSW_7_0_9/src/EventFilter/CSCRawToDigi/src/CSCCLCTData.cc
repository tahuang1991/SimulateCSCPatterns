...
void CSCCLCTData::printPattern(uint32_t id) 
{
    
    // if me1/1
    std::cout << "printPattern: CLCT data in CSC id = " << id << "( station = " << CSCDetId::station(id) << " ring = " << CSCDetId::ring(id) << " chamber = " << CSCDetId::chamber(id) << ")" << std::endl;
    if (CSCDetId::station(id)==1 && (CSCDetId::ring(id)==4 || CSCDetId::ring(id)==1)) {
        int nMaxLayersWithHit = 0;
        std::cout << "printPattern: begin"<< std::endl;
        for (int tbin = 0; tbin < ntbins_; tbin++) {
            int nLayersWithHit = 0;
            for (int layer = 1; layer < 7; layer++) {
                printf("%02d", tbin);
                std::cout << " " << layer-1 << " |";
                bool isLayerWithHit = false;
                for (int cfeb = 0; cfeb < 5; cfeb++) {
                    for (int distrip = 0; distrip < 8; ++distrip) {
                        std::cout << bitValue(cfeb, tbin, layer, distrip);
                        if ( bitValue(cfeb, tbin, layer, distrip) ) isLayerWithHit = true;
                    }
                    std::cout << "|";
                }
                std::cout << std::endl;
                if ( isLayerWithHit ) nLayersWithHit++;
            }
            if ( nLayersWithHit > nMaxLayersWithHit ) nMaxLayersWithHit = nLayersWithHit;
        }
        std::cout << "printPattern: max number of layers with hits = " << nMaxLayersWithHit << std::endl;
        std::cout << "printPattern: end"<< std::endl;
    }
...
//    if (CSCDetId::station(id)==1 && (CSCDetId::ring(id)==4 || CSCDetId::ring(id)==1)) {
//        for (int cfeb = 0; cfeb < 5; cfeb++) {
//            for (int tbin = ntbins_-1; tbin > -1; --tbin) {
//                //hits = 0;
//                for (int layer = 1; layer < 7; layer++) {
//                    for (int distrip = 0; distrip < 8; ++distrip) {
...
//        }
//        if (pass > 0) {
//            std::cout << "station = " << CSCDetId::station(id) << ", chamber = " << CSCDetId::chamber(id) << std::endl;
//            for (int cfeb = pcfeb; cfeb < pcfeb+1; cfeb++) {
//                for (int tbin = pass; tbin < ntbins_; ++tbin) {
//                    std::cout << "cfeb = " << cfeb << ", tbin = " << tbin << std::endl;
//                    for (int layer = 1; layer < 7; layer++) {
//                        for (int distrip = 0; distrip < 8; ++distrip) {
//                            std::cout << bitValue(cfeb, tbin, layer, distrip);
//                        }
