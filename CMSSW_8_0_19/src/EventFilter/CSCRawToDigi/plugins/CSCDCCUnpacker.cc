      ...
      if (length>=32)  ///if fed has data then unpack it
        {
		  ...
		  // if (cscData.size() != 0) std::cout << "FED" << id << " DDU Source ID: " << dduData[iDDU].header().source_id() << " firmware version: " << dduData[iDDU].header().format_version() << std::endl;
		      ...
		      ///
                      if (goodTMB)
                        {
			  //print out strip digis: tri-ads, only for ME11, including ME1a and ME1b
        	          cscData[iCSC].clctData()->printPattern(pcrate->detId( vmecrate, dmb, 0, 0 ).rawId());
				  ...
				  //check ME11, including ME1a and ME1b
				  if (layer.station()==1 and layer.ring()==1 and correlatedlctDigis[i].isValid())
				      std::cout <<"CSCid "<< layer << " "<< correlatedlctDigis[i] << std::endl;
                                }
